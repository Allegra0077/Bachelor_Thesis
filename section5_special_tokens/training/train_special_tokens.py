"""
Train learned special tokens for user-message prediction (Section 5.3).

Adds k new tokens to a pretrained model's vocabulary, freezes all existing
parameters, and trains only the new token embeddings to minimize cross-entropy
loss on predicting user messages from dialogue context. Supports single- and
multi-conversation training, token count sweeps, and conversation count sweeps.

Example usage:
    # Single training run
    python train_special_tokens.py \
        --train_examples_path train_examples.jsonl \
        --test_examples_path test_examples.jsonl \
        --synthetic_user_id bp_career_builder__st_formal \
    ...
    python train_special_tokens.py \
        ...
        --test_examples_path test_examples.jsonl \
        --synthetic_user_id bp_career_builder__st_formal \
        --run_token_count_sweep \
        --token_count_values 1 3 5 10 20

    # Conversation count sweep
    python train_special_tokens.py \
        --train_examples_path train_examples.jsonl \
        --test_examples_path test_examples.jsonl \
        --synthetic_user_id bp_career_builder__st_formal \
        --token_count 10 \
        --run_num_conversations_sweep \
        --num_conversation_values 1 2 4 6 8
"""
