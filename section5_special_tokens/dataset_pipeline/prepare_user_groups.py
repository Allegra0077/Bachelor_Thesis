"""
Group conversations by synthetic user ID (Section 5.2).

Reads a JSONL file of generated conversations and groups them by
synthetic user (base_persona_id + style_id). Output is used by
create_user_splits.py to produce train/test splits.

Example usage:
    python prepare_user_groups.py \
        --input_path conversations.jsonl \
        --output_path grouped_users.json
"""
...
def main():
    ...
    parser.add_argument("--input_path", type=str, required=True, help="Path to conversation JSONL")
    parser.add_argument("--output_path", type=str, required=True, help="Path to grouped users JSON")
