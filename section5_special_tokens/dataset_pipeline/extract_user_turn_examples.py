"""
Extract per-turn training and test examples from conversations (Section 5.2).

For each conversation, extracts individual examples where:
  - target = a non-seed user turn
  - context = all preceding messages in the conversation

Uses the train/test splits from create_user_splits.py to separate
examples into training and test JSONL files.

Example usage:
    python extract_user_turn_examples.py \
        --conversations_path conversations.jsonl \
        --splits_path user_splits.json \
        --output_train_path train_examples.jsonl \
        --output_test_path test_examples.jsonl
"""
...
def extract_examples_from_conversation(conv: dict):
    """
    Extract examples where target is a user turn, excluding the first seed turn.
